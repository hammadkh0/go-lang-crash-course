...
func timer() func() {
	start := time.Now()
	// this return function will be the defered call
	return func() {
		fmt.Println("Main funciton took", time.Since(start))
	}
}

func main() {
	defer timer()() // defer the function to get the time difference between start and end of the function. The trailing () is the defered call
